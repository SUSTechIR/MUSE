...
class PathCon(nn.Module):
    ...
    def _call_model(self):
        ...
        entity_bert_feature = torch.index_select(self.node_features, 0, self.entity_pairs.view(-1)).view(self.batch_size, 2, -1) #[batch_size, 2, node_dim]
        if self.use_context:
            entity_bert_feature = self.node_linearlayer(entity_bert_feature) #[batch_size, 2, relation_dim]

        self.scores+=F.relu(self.node_linearlayer2(torch.index_select(self.node_features, 0, self.entity_pairs.view(-1)).view(self.batch_size, -1)))

        if self.use_context:
            edge_list, mask_list = self._get_neighbors_and_masks(self.labels, self.entity_pairs, self.train_edges)
            self.aggregated_neighbors = self._aggregate_neighbors(edge_list, mask_list, entity_bert_feature)
    ...
    def _build_node_feature(self):
        print("begin build node feature")
        # bert = np.load("../data/"+self.dataset+"/"+"node_features.npy")
        # bert = np.load("../data/"+self.dataset+"/"+"entity_features.npy")

        bert = np.load("../data/"+self.dataset+"/"+"entity_features_pretrained.npy")
        self.node_dim = bert.shape[1]
        self.node_features = torch.tensor(bert).to(torch.float32).cuda() if self.use_gpu \
            else torch.tensor(bert).to(torch.float32)
        
        self.node_features = torch.cat([self.node_features,
            torch.zeros([1,self.node_dim]).cuda() if self.use_gpu \
                else torch.zeros([1,self.node_dim])], dim=0)
        if self.use_context:
            self.node_linearlayer = nn.Linear(self.node_dim, self.relation_dim)
            nn.init.xavier_uniform_(self.node_linearlayer.weight)

        self.node_linearlayer2 = nn.Linear(self.node_dim * 2, self.n_relations)
        nn.init.xavier_uniform_(self.node_linearlayer2.weight)
        print("end build node feature")
    ...
    def _get_neighbors_and_masks(self, relations, entity_pairs, train_edges):
        ...
        Return
        -------
        edges_list: 存着临边，[0跳，1跳，2跳] 是samples，每一跳存的是edges_id
        mask: 把edges_id中relation为训练的relation的删去
        """
    ...
    def _get_neighbor_aggregators(self):
        ...
        if self.context_hops == 1:
            ...
            aggregators.append(self.neighbor_agg(batch_size=self.batch_size,
                                                 ...
                                                 output_dim=self.n_relations,
                                                 self_included=False))
            # aggregators.append(LastConcatAggregator(batch_size=self.batch_size,
            #                                         input_dim=self.hidden_dim,
            #                                         output_dim=self.n_relations,
            #                                         self_included=True))
    ...
    def _aggregate_neighbors(self, edge_list, mask_list, entity_bert_feature):
        ...
        for i in range(self.context_hops):
            ...
            for hop in range(self.context_hops - i):
                if(i == 0):
                    vector = aggregator(self_vectors=[edge_vectors[hop] ,entity_bert_feature],
