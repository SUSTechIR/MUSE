...
def read_triplets(file_name):
    """
    返回 [[head_idx, tail_idx, relation_idx], ...]
    """
    data = []
...
def get_paths(train_triplets, valid_triplets, test_triplets):
    ...
    if os.path.exists(directory + 'train_paths_' + length + '.pkl'):
        ...
        ## 有向,(h,t) pair 包含所有集合里面的相邻的pair, (h,t)是所有的，path只在train里面
...
def load_data(model_args):
    ...
    entity_dict = read_entities(directory + 'entities.dict') ##name : id
    relation_dict = read_relations(directory + 'relations.dict') ##name : id
    ...
    if args.use_context:
        neighbor_params = [np.array(entity2edges), np.array(edge2entities), np.array(edge2relation), len(entity_dict), entity2edge_set]
    else:
